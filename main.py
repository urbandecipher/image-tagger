...
async def scan_folder(req: ScanRequest):
    ...
    return {"total": total, "new": new_count, "untagged": untagged}
...
async def start_tagging(req: TagRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        run_tagging_job, req.folder, req.threshold, req.batch_size
    )
...
async def list_images(
    ...
    # Decode URL-encoded spaces (+ -> space), then split by comma only
    query = query.replace("+", " ")
    tags = [t.strip() for t in re.split(r",", query) if t.strip()]
    images, total = db.search_images(tags, page, per_page, untagged_only)
    ...
    for i in range(0, len(untagged), batch_size):
        ...
        try:
            results = await loop.run_in_executor(
                None, tagger.tag_images, paths, threshold
            )
